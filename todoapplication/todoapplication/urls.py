...
from task.views import TodoCreateView,TodoListView,TodoDetailView,TodoDeleateView,TodoUpdateView

urlpatterns = [
    path('admin/', admin.site.urls),
    path("todos/add",TodoCreateView.as_view(),name="todo-add"),
    path("todos/all",TodoListView.as_view(),name="todo-list"),
    path("todos/<int:pk>",TodoDetailView.as_view(),name="todo-details"),
    path("todos/<int:pk>/remove",TodoDeleateView.as_view(),name="todo-remove"),
    path("todos/<int:pk>/change",TodoUpdateView.as_view(),name="todo-change"),
    path("",include("reminder.urls")),
    path("api/",include("taskapi.urls")),
    path("api/v2.1/",include("reminderapi.urls")),
